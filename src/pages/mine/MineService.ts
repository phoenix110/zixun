...
export class MineService {
  ...
  //获取当前积分/user/score demand/xieyi
  getDemandCount1(){
    return this.httpService.post( '/user/score').map((res: Response) => res.json());
  }

  //点击签到按钮
  getDemandCount2(){
        return this.httpService.post( '/user/sign').map((res: Response) => res.json());
    }
